...
## 한/영 두 벌 콘텐츠 — 영문 칸은 비워 두라고 있는 것이 아니다

운영진이 입력하는 콘텐츠는 `_ko`/`_en` 두 벌이다(`title_ko`·`description_ko`·
`prep_notes_ko`…). **한 벌짜리 칸을 새로 만들지 말 것.** 공연의 '준비물 · 복장 · 안내'가
오래 한 칸이었는데, 이 글은 공연 전날 안내 메일과 회람(`/rsvp`) 공지, 회원 상세에 그대로
실린다 — 영어로 읽는 가족에게는 **무엇을 챙겨 와야 하는지가 통째로 빠진 메일**이 갔다.
(0046이 `prep_notes` → `prep_notes_ko` + `prep_notes_en`으로 갈랐다.)

규칙:

1. **영문이 비면 한국어로 물러선다.** 화면은 `LocaleText`(또는 `useLocaleText`), 메일은
   템플릿에서 `s(data, 'noteEn', s(data, 'note'))`처럼 폴백을 적는다. 영문이 없다고
   그 줄을 통째로 빼지 않는다 — 없는 것보다 읽히지 않는 것이 낫다.
2. **영문 초벌은 사람이 손으로 옮겨 적지 않는다.** `components/admin/AiTranslateFields.tsx`
   한 조각이 그 일을 한다. 채울 칸 목록만 넘기면 된다(공연 폼이 현재 유일한 사용처이고,
   수업·뉴스도 같은 방식으로 붙이면 된다):

   ```tsx
   <AiTranslateFields
     onApply={patchForm}
     fields={[{ key: 'prep_notes_en', label: '준비물 · 복장 · 안내',
                ko: f.prep_notes_ko, en: f.prep_notes_en }, …]}
   />
   ```

3. **한 번에 한 요청으로 여러 칸을 함께 보낸다.** 제목·준비물·설명은 같은 공연을 말한다.
   따로 부르면 작품 이름이 칸마다 달라진다(Samgomu / Three-Drum Dance / Samgo-mu).
4. **AI 용도는 `text.polish`(문구 다듬기 · 번역 보조)** 하나다 — 모델은 관리 콘솔 >
   AI 설정이 정한다. 판단(프롬프트·응답 해석)은 `lib/ai/translate.ts`에 있고
   `translate.test.ts`가 잠근다. 새 정규식·새 프롬프트를 라우트 안에 쓰지 말 것.
5. **번역은 저장하지 않는다.** 결과는 폼에만 채우고 저장 버튼이 확정한다. 기계 번역이
   사람의 확인 없이 학부모에게 나가는 길을 만들지 않는다. 이미 적힌 영문은 기본으로
   건드리지 않는다(덮으려면 체크박스를 켠다 — 무엇이 일어날지 누르기 전에 화면이 말한다).
6. **칸 이름은 맥락이지 번역 대상이 아니다.** 라벨과 원문을 한 덩이로 보냈더니 모델이
   라벨을 제목으로 읽고 "What to Bring · Attire · Information"을 번역문 맨 앞에 붙여
   돌려줬다 — 운영진이 적지 않은 제목이 안내문에 생긴다. 지금은 `<<<원문>>>` 울타리로
   가른다. 모델이 **원문을 그대로 반사한 칸도 버린다**(영문 칸에 한국어가 앉으면 화면은
   "영문 있음"으로 보인다).
...
### 사용법 안내(`/admin/resources/guide`) — 캡처는 가짜 자료함으로 찍는다

7. **안내 페이지의 캡처는 진짜 자료함을 찍지 않는다.** 진짜를 찍으면 실제 번호와
   비밀번호가 안내에 박제되고, 그 둘이 저작권 자료를 여는 열쇠다. 지그(`shotjig`)를
   임시로 세워 **진짜 컴포넌트를 가짜 props로 렌더**해서 찍고 지웠다 — 가짜 번호
   `473128`·`820517`·`361094`는 실제 코드와 겹치지 않음을 확인한 값이다.
   다시 찍는 절차는 `app/admin/resources/guide/page.tsx` 머리말에 있다.
8. **캡처와 그림의 역할이 다르다.** 우리 화면은 남의 상표가 찍히지도, 우리 모르게
   바뀌지도 않으므로 **캡처가 정직하다**(유튜브 안내와 반대). 인라인 SVG
   (`VaultGuideFigures`)는 **캡처로 찍을 수 없는 것**만 맡는다 — 전체 흐름과
   "번호는 주소, 비밀번호는 열쇠". 둘 다 어느 한 화면에도 들어 있지 않다.
9. **버튼 이름은 화면 글자 그대로 쓴다.** locale 오버라이드 때문에 코드의 폴백과
   화면이 다를 수 있다(코드는 '만들기'인데 화면은 **'생성'**). 안내를 쓸 땐 반드시
   **화면을 열어 확인**할 것 — 다르면 안내가 아니라 수수께끼가 된다.
10. **`expiresAt`은 데이터에만 있고 설정 UI가 없다.** 목록이 '기간 지남'을 보여 주긴
    하지만 기간을 정하는 자리가 없으니, 안내에 만료 기능을 약속하지 말 것.
