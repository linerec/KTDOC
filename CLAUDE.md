...
3. **'퍼가기 허용'이 꺼진 영상을 등록 전에 막는다.** 링크는 멀쩡한데 사이트에서는
   오류만 나오는 영상이 있다(실제로 공연 영상 하나가 그 상태였다). 올린 사람은 모르고
   방문자만 깨진 화면을 본다. 등록 화면은 이미 붙어 있는 영상도 한 번에 점검한다
   (`checkYouTubeVideos`, Data API 1회).
