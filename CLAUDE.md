...
5. **재생은 누르기 전에 유튜브를 부르지 않는다.** 썸네일 한 장만 두고 누르는 순간
   재생기를 끼운다(facade). 도메인은 `youtube-nocookie.com`. 공개 화면의 임베드는
   `youtubeEmbedUrl()`을 쓰고, 세로면 상자에 `is-short`를 붙인다 — 9:16은 **폭이 아니라
   높이로** 묶는다(폭만 제한하면 격자 줄 높이를 밀어 올려 옆 칸 아래가 빈다).
