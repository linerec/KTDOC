...
작성일: 2026-07-27 · **상태: 구현 완료 (2026-07-27)** · 구현 계획: `school-events-plan.md`
...
> **구현 중 확인된 사실(중요):** 메뉴 권한은 **이미 열려 있었다.** MySQL
> `menu_permissions`에 `gallery`/teacher = 1, `gallery.photos`/teacher = 1 행이
> 존재한다(레지스트리 `defaultRoles`가 `['admin']`이어도 DB 값이 우선한다).
> 진짜 막고 있던 것은 **API였다** — `/api/admin/gallery/**`가 전부 `isAdmin`이라
> 선생님은 화면에는 들어가지만 저장하면 403을 받는 상태였다. 즉 기존에 실사용
> 버그가 있었다.

조치: 생성·수정 계열 API를 `isStaff`로 열고, 되돌릴 수 없는 삭제는 `isAdmin`을 유지한다.

| 열림(`isStaff`) | 유지(`isAdmin`) |
|---|---|
| 이벤트 목록·상세·생성·수정 | 이벤트 삭제 |
| 이미지 업로드, 이미지 순서 변경 | 이미지 삭제 |
| 영상 추가, 영상 순서 변경 | 영상 삭제 |
| | 사진 보관함 일괄 작업, 카테고리 CRUD |

레지스트리 `defaultRoles`에도 `teacher`를 추가했다(DB에 행이 없는 환경에서의 폴백 일치).

안전장치는 이미 구조에 있다 — `is_published` 기본값이 0이므로 선생님은 초안까지
만들고 공개는 admin이 판단한다.

**남은 트레이드오프:** 선생님이 자기가 방금 올린 사진도 지울 수 없다(삭제가 admin
전용이므로). 소유권 기반 삭제는 범위를 넘어서므로 넣지 않았다. 운영하며 불편이
확인되면 "본인이 올린 것만 삭제"를 별도로 검토한다.
...
**구현 중 확인된 캐시 이슈:** 홈은 정적 프리렌더(ISR)된다. `lib/youtube.ts:97`의
`revalidate: 3600`이 페이지 전체를 1시간에 묶고 있어서, 행사를 공개해도 홈에
최대 1시간 늦게 떴다 — 운영자가 "공개했는데 홈에 안 보인다"고 오해하기 딱 좋은
지점이다. `app/page.tsx`에 `export const revalidate = 300`(5분)을 두어 해소했다.
YouTube 응답은 fetch 레벨에서 계속 캐시되므로 늘어나는 비용은 5분당 D1 조회 한 번뿐이다.
