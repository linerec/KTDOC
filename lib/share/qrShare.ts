 ...
 * 화면에 글자로 보여 줄 주소.
 *
 * **QR에 담는 주소와 사람에게 보여 주는 주소는 다르다.** QR은 스캐너가 읽으므로
 * 한글 경로가 퍼센트 인코딩돼 있어야 하는데, 그 상태 그대로 화면에 찍으면
 *
 *   ktdoc.org/gallery/2024/2024-%EB%89%B4%EC%A0%80%EC%A7%80-%EC%B6%94%EC%84%9D…
 *
 * 처럼 알아볼 수 없는 긴 줄이 된다. 카톡으로 옮기려고 눈으로 확인하는 자리인데
 * 무엇을 보내는지 알 수 없게 된다. 그래서 보여 줄 때만 되돌린다.
 *
 * - `decodeURI`는 구분자(`/ ? # &`)는 건드리지 않아 주소 구조가 깨지지 않는다.
 * - 잘못된 시퀀스가 섞이면 던지므로, 그때는 원문을 그대로 보여 준다
 *   (읽기 어려울지언정 틀린 주소를 보여 주지는 않는다).
 * - `https://`는 뗀다. 공유 주소는 늘 https라 붙여 봐야 자리만 먹는다.
 */
