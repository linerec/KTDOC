 ...
 * 이벤트 연결 시 event_images를 자동 생성/이동/삭제하고, 삭제 시 R2 객체까지 정리한다.
 */
 ...
 * - 이벤트 연결 해제: 기존 event_image 삭제
 * - 이벤트 (재)연결: 필요 시 기존 event_image 제거 후 새 event_image 생성·연결
...
export async function organizePhoto(
  ...
  // 이벤트 연결 해제 → 연결돼 있던 event_image 정리
  ...
  if (shouldAttachToEvent) {
    // 다른 이벤트로 옮기는 경우 기존 event_image 먼저 제거
