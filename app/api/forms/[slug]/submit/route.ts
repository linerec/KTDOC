 ...
 *
 * ──────────────────────────────────────────────────────────────────
 * **쌍둥이가 있다: app/api/admin/forms/[id]/responses (대리 입력).**
 *
 * 문항을 고치는 일은 여기와 무관하다 — 화면(FormRenderer)·검증(validateAnswers)·
 * 저장(insertResponse)을 둘이 함께 쓰므로 스키마만 고치면 양쪽에 같이 반영된다.
 * 갈라지는 것은 **이 파일의 단계를 늘릴 때**다. 위 1~7 중 하나를 더하거나 고치면
 * 그것이 전화로 받은 신청에도 필요한 일인지 그쪽에서 판단해야 한다.
 * (예: 접수 확인 메일 · 운영진 푸시 · 형제 할인 계산 같은 것)
 *
 * 지금 일부러 갈라 둔 것: 스팸 3종(운영진은 검사하지 않는다) · 접수기간·로그인
 * 게이트(마감 뒤 전화 신청을 받아야 한다) · 신청서 안 회원가입(남의 계정을
 * 운영진이 만들지 않는다) · 이메일 필수(lib/forms/staffEntry.ts 가 규칙을 갖는다).
 * ──────────────────────────────────────────────────────────────────
 */
