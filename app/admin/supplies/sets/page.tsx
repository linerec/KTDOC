/**
 * Admin Supply Sets List (준비물 세트)
 */

import Link from 'next/link';
import { auth } from '@/auth';
import { requireMenuAccess } from '@/lib/admin/permissions';
import { getSupplySets } from '@/lib/d1';
...
export default async function AdminSupplySetsPage({ searchParams }: PageProps) {
  ...
  const { sets, total } = await getSupplySets({
    ...
    <div className="admin-page">
      <div className="admin-header">
        <div className="admin-header-content">
          ...
          <h1 className="admin-title">준비물 세트</h1>
          <p className="admin-subtitle">
            자주 함께 챙기는 준비물을 하나로 묶습니다. 이벤트·수업에서는 개별 항목과 세트를 섞어 지정할 수 있습니다.
          </p>
        </div>
        <div className="admin-header-actions">
          <Link href="/admin/supplies" className="admin-btn admin-btn-outline">
            개별 항목
          </Link>
